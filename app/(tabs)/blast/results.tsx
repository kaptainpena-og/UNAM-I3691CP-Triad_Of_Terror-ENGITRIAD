...
export default function BlastResults() {
  ...
  const N = parseFloat(params.holesPerDelay as string) || 0;
  const D = parseFloat(params.distanceToStructure as string) || 1;
  const K = parseFloat(params.siteConstantK as string) || 682;
  const alpha = parseFloat(params.attenuationAlpha as string) || 1.6;
  ...
  let technicalSummary = 'Vibration levels remain safely within structural safety envelopes. High certainty of zero environmental damage risks.';

  if (predictedPPV >= 50.0) {
    diagnosticStatus = 'IMMINENT FAILURE CRITICAL RISK';
    designColorCode = '#ef4444';
    technicalSummary = 'Critical danger warning flag active! Severe structural cracks and masonry breakage probable. Immediately drop charge load arrays.';
  } else if (predictedPPV >= 19.0) {
    diagnosticStatus = 'CAUTIONARY HAZARD ALERT';
    designColorCode = '#f97316';
    technicalSummary = 'Threshold parameters border architectural tolerances. Prolonged exposure will result in cosmetic wall plaster fracturing.';
  } else if (predictedPPV >= 5.0) {
    diagnosticStatus = 'MINIMAL WARNING TIERS';
    designColorCode = '#eab308';
    technicalSummary = 'Vibrations will be strongly perceptible to nearby civilian occupants. Structural integrity remains secure.';
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.appSubtitle}>PREDICTIVE WAVE VIBRATION OUTCOMES</Text>
      </View>
      ...
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>• CALCULATED PEAK PARTICLE VELOCITY</Text>
          <View style={styles.mainScoreWrapper}>
            <Text style={styles.velocityValueText}>{predictedPPV.toFixed(2)}</Text>
            <Text style={styles.velocityUnitText}>mm / s</Text>
          </View>
          <View style={[styles.statusBand, { backgroundColor: designColorCode }]}>
            <Text style={styles.statusBandText}>STRUCTURAL TIER: {diagnosticStatus}</Text>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.logItem}>
            ...
            <Text style={styles.logValue}>{scaledDistance.toFixed(2)} m/kg^0.5</Text>
          </View>

          <View style={styles.logItem}>
            <Text style={styles.logLabel}>Geological Attenuation Ratio (K / α)</Text>
            <Text style={styles.logValue}>{K} / {alpha}</Text>
          </View>
        </View>

        {/* Dynamic Contextual Advisory Panel */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>• BLAST ENGINEERING ADVISORY RECOMMENDATIONS</Text>
        ...
        <TouchableOpacity style={styles.backButton} onPress={() => router.back()}>
          <Text style={styles.backButtonText}>◀ ADJUST PATTERN INITIAL DESIGN</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  appTitle: { fontSize: 19, fontWeight: '900', color: '#00bfff', letterSpacing: 1.2 },
  ...
  cardTitle: { fontSize: 12, fontWeight: 'bold', color: '#00bfff', marginBottom: 14 },
  mainScoreWrapper: { alignItems: 'center', paddingVertical: 12 },
  velocityValueText: { fontSize: 42, fontWeight: '900', color: '#ffffff', fontFamily: 'monospace' },
  velocityUnitText: { fontSize: 12, color: '#88aabb', fontWeight: 'bold', marginTop: 4 },
  statusBand: { padding: 8, borderRadius: 4, marginTop: 12 },
  statusBandText: { color: '#ffffff', fontSize: 12, fontWeight: 'bold', textAlign: 'center' },
  logItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#112544' },
  logLabel: { color: '#aabbcc', fontSize: 12 },
  logValue: { color: '#ffffff', fontSize: 13, fontWeight: '700', fontFamily: 'monospace' },
